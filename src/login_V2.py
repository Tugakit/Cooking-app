...
def Exit():
    result = tkMessageBox.askquestion('System', 'Vous etes sur de quiiter?', icon="warning")
    if result == 'yes':
        root.destroy()
        exit()
...
def LoginForm():
    ...
    lbl_username = Label(LoginFrame, text="Nom d'Utilisateur:", font=('arial', 25), bd=18)
    lbl_username.grid(row=1)
    lbl_password = Label(LoginFrame, text="Mot de passe:", font=('arial', 25), bd=18)
    ...
    btn_login = Button(LoginFrame, text="Se connecter", font=('arial', 18), width=35, command=Login)
    btn_login.grid(row=4, columnspan=2, pady=20)
    lbl_register = Label(LoginFrame, text="S'enregistrer", fg="Blue", font=('arial', 12))
...
def RegisterForm():
    ...
    lbl_username = Label(RegisterFrame, text="Nom d'utilisateur:", font=('arial', 18), bd=18)
    lbl_username.grid(row=1)
    ...
    btn_login = Button(RegisterFrame, text="S'enregistrer", font=('arial', 18), width=35, command=Register)
    btn_login.grid(row=6, columnspan=2, pady=20)
    lbl_login = Label(RegisterFrame, text="Se connecter", fg="Blue", font=('arial', 12))
...
def Register():
    ...
    if USERNAME.get == "" or PASSWORD.get() == "" or FIRSTNAME.get() == "" or LASTNAME.get == "":
        lbl_result2.config(text="Veuillez remplir tous les champs!", fg="orange")
    else:
        cursor.execute("SELECT * FROM `member` WHERE `username` = ?", (USERNAME.get(),))
        if cursor.fetchone() is not None:
            lbl_result2.config(text="Nom d'utilisateur déjà utilisé", fg="red")
        else:
            ...
            FIRSTNAME.set("")
            LASTNAME.set("")
            lbl_result2.config(text="Enregistrement terminé !!", fg="black")
        cursor.close()
        conn.close()
...
def Login():
    ...
    if USERNAME.get == "" or PASSWORD.get() == "":
        lbl_result1.config(text="Veuillez remplir tous les champs!", fg="orange")
    else:
        cursor.execute("SELECT * FROM `member` WHERE `username` = ? and `password` = ?",
                       (USERNAME.get(), PASSWORD.get()))
        if cursor.fetchone() is not None:
            lbl_result1.config(text="Connection terminée", fg="blue")
        else:
            lbl_result1.config(text="Npm d'utilisateur ou mot de passe érroné", fg="red")
